...
- **Frame decoding that stays out of the way**：filmstrip tiles decode with per-tile bounded tolerances（an unbounded decode snaps to a keyframe, and x264 puts keyframes on scene changes, which would show the *next* scene）, exact cells load outward from the frame you're looking at, and both caches evict a slice at a time so a hover sweep never triggers a wall of re-decodes.
